...
declare
  ...
  v_locations text[] := array[
    'London', 'Manchester', 'Birmingham', 'Bristol', 'Leeds', 'Brighton', 'Glasgow',
    'Edinburgh', 'Cardiff', 'Liverpool', 'Sheffield', 'Nottingham', 'Oxford', 'Cambridge',
    'Reading', 'Guildford', 'Surrey', 'Essex', 'Kent', 'Hampshire'
  ];
  v_genres text[] := array[
...
begin
  ...
  for i in 1..50 loop
    ...
    values (
      ...
      v_primary,
      format(
        '%s player based in %s. Experienced with %s and %s gigs across the UK.',
        v_primary,
        v_location,
        v_genre_a,
        v_genre_b
